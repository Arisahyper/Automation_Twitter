...
class User:
    ...
    def Timeline_favorite(self):
        ...
        for tweet in timeline:
            if not "@" in tweet.text[0]:  # noqaE713
                tweet_id = tweet.id
                    ...
                    try:
                        if a == "f":
                            ...
                            try:
                                ...
                                print("既にいいね済みかいいねをつけることができませんでした", e)
                                sleep(2)
                                # subprocess.run('clear')
                        ...
                        if a == "r":
                            ...
                            try:
                                ...
                                print("既にリツイート済みかリツイートをつけることができませんでした", e)
                                sleep(2)
                                # subprocess.run('clear')
...
class Hololive(User):
    ...
    def create_user_graph(self, count):  # 特定のユーザーのツイート取得しツイート
        ...
        for tweet in tweets:
            if not "RT @" in tweet.text[0:4]:  # noqaE713
                try:
        ...
        #     status=(f"@ {account} ReactionChart"), filename=imgname
        # )

    def favorite_by_fanname(self, count):
        search_results = api.search(
            q=self.fan_name + " -filter:retweets", count=count
        )
        now_count: int = 0

        for result in search_results:
            if not "@" in result.text[0]:  # noqaE713
                user_name = result.user.screen_name
                user_id = result.id
                user = result.user.name
                tweet = result.text
    ...
    def favorite_by_livetag(self, count):
        search_results = api.search(
            q=self.live_tag + " -filter:retweets", count=count
        )
        now_count: int = 0
    ...
    def favorite_by_cuttag(self, count):
        search_results = api.search(
            q=self.cut_tag + " -filter:retweets", count=count
        )
        now_count: int = 0
